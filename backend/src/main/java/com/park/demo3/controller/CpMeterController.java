...
public class CpMeterController {
    ...
    @Operation(summary = "有充电记录的账期('YYYY-MM' 升序;空表=[]。vehicleType 非空=只数该车型的桩)")
    @GetMapping("/months")
    public List<String> months(@RequestParam(required = false) String vehicleType) {
        return svc.months(vehicleType);
    }
