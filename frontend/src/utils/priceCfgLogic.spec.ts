...
describe('PRICE_KEYS 注册表(§2,V62 定格 19 键 + 包干两键 + S14 green_rate)', () => {
  it('21 键且唯一(五个月推键与改名前的 elevator_package 都不在)', () => {
    expect(PRICE_KEYS).toHaveLength(21)
    expect(new Set(PRICE_KEYS.map(k => k.key)).size).toBe(21)
  ...
  it('户级可覆盖集合=六键+S14 green_rate', () => {
    expect(PRICE_KEYS.filter(k => k.overridable).map(k => k.key)).toEqual(
      ['mgmt_fee', 'capacity_fee', 'water', 'elec_package', 'share_elec_fixed', 'share_water_fixed', 'green_rate'])
  })
})
...
describe('buildVersionStatus 版本状态组装(右栏卡+徽标)', () => {
  ...
  it('各组生效键计数(常数前滚计入;户级行不计)', () => {
    ...
    expect(g('特殊轨道')).toMatchObject({ effective: 0, total: 5 })   // +包干两键+green_rate(S14) −改名前的 elevator_package
  })
