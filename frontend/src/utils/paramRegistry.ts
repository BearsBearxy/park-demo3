...
export const PARAM_DEFS: ParamDef[] = [
  ...
  // 外部天气数据源断闸三键(PV-ANALYSIS-SPEC §02)。顺序与后端 ParamRegistry static{} 逐位对齐 —— spec 按下标比对
  { key: 'weather_api_enabled', label: '外部天气API 总开关', unit: '', group: 'constant', defaultMode: 'from', monthlyCheck: false, valueKind: 'bool',
    hint: '关闭后立即停止一切外部请求，分析屏自动回落相对口径（园区内部互相当基准）' },
  { key: 'weather_api_monthly_cap', label: '外部天气API 月调用上限', unit: '次', group: 'constant', defaultMode: 'from', monthlyCheck: false, valueKind: 'int',
    formula: '本月调用数达到上限自动关闭总开关并记日志',
    hint: '防的是「忘了关」，不是「想关」' },
  { key: 'weather_source', label: '天气数据源', unit: '', group: 'constant', defaultMode: 'from', monthlyCheck: false, valueKind: 'enum',
    enumOptions: { 0: '导入（不发外部请求）', 1: '和风天气 API', 2: 'datashareclub API' },
    hint: '默认走导入中心，不产生任何费用' },
