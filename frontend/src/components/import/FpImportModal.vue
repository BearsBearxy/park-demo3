...
.fpimp-scrim { position:fixed; inset:0; z-index:320; background:rgba(28,28,28,.32); backdrop-filter:blur(2px); display:flex; justify-content:flex-end; }
.fpimp { width:min(560px,96vw); height:100%; background:var(--surface-white); box-shadow:-12px 0 40px rgba(28,28,28,.18); display:flex; flex-direction:column; animation:fpimpslide .22s var(--ease-standard); }
@keyframes fpimpslide { from { transform:translateX(28px); opacity:.4; } to { transform:none; opacity:1; } }
