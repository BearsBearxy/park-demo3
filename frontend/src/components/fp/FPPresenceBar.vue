<script setup lang="ts">
// 顶栏在场头像组(PRESENCE 设计稿 §03;2026-09-18 放大改版见 TAB-BAR-SPEC §7)。
//
// ⚠ **宽度按满员算死**(4 头像 + 溢出位 + 「N 人在线」= 256px):一个人在线和八个人在线时工具条
//   不挪一个像素。LAYOUT-STABILITY-SPEC 的硬要求 —— 顶栏是全站最不该动的一条。
//
// 自己不进头像组(你知道自己在哪),只在名单里置顶;但「N 人在线」的 N 包括自己 —— 和名单人数一致。
import { ref, computed } from 'vue'
import { Users } from 'lucide-vue-next'
import { usePresenceStore, type Seat } from '@/stores/presence'
import Avatar from '@/components/ds/Avatar.vue'
import Popover from '@/components/ds/Popover.vue'
import ShellTip from '@/components/shell/ShellTip.vue'
import { personNick } from '@/utils/personNick'

const presence = usePresenceStore()
const open = ref(false)

const MAX = 4
const shown = computed(() => presence.others.slice(0, MAX))
const overflow = computed(() => Math.max(0, presence.others.length - MAX))
const editingCount = computed(() => presence.users.filter((u) => u.mode === 'edit').length)
/** 名单里自己置顶 —— 「你」是定位锚点，先告诉他自己在哪，再看别人。 */
const rows = computed(() => [...presence.users].sort((a, b) => Number(b.self) - Number(a.self)))

/** 空闲：距上次心跳超过一拍半 —— 快掉线了，淡显但先别抹掉。 */
const fading = (u: Seat) => u.idleMs > 30_000
const stateText = (u: Seat) =>
  u.mode === 'edit' ? `编辑 ${mmss(u.sinceMs)}` : fading(u) ? '刚离开' : '浏览中'
/** 悬停一个头像时的那句话:名字 + 在干什么 + 在哪一屏。 */
const tipState = (u: Seat) =>
  u.mode === 'edit' ? `正在编辑 · ${mmss(u.sinceMs)}` : fading(u) ? '刚离开' : '浏览中'
function mmss(ms: number): string {
  const s = Math.max(0, Math.floor(ms / 1000))
  return `${String(Math.floor(s / 60)).padStart(2, '0')}:${String(s % 60).padStart(2, '0')}`
}
</script>

<template>
  <Popover v-model="open" :width="340" align="end">
    <template #trigger>
      <button class="pb" type="button" :aria-label="`在线 ${presence.users.length} 人`" @click="open = !open">
        <span v-if="presence.others.length" class="pb-stack">
          <ShellTip v-for="u in shown" :key="u.sid" :title="`${u.displayName}　${tipState(u)}`" :sub="u.label || ''" :disabled="open">
            <Avatar :uid="u.user" :name="u.displayName" :text="personNick(u.displayName)" :size="30"
                    :class="['pb-av', u.mode === 'edit' ? 'edit' : 'view', { fade: fading(u) }]" />
          </ShellTip>
          <span v-if="overflow" class="pb-more">+{{ overflow }}</span>
        </span>
        <ShellTip v-if="presence.others.length" title="现在谁在线" sub="橙圈的人正在编辑" align="end" :disabled="open">
          <span class="pb-n"><b>{{ presence.users.length }}</b> 人在线</span>
        </ShellTip>
        <span v-else class="pb-n pb-alone">只有你在线</span>
        <!-- L 以下的化身:人形图标 + 人数。CSS 按档显隐,不走 JS(RESPONSIVE-LAYOUT-SPEC §3.2) -->
        <span class="pb-count"><Users :size="16" /><b>{{ presence.users.length }}</b> 人</span>
      </button>
    </template>

    <div class="pp">
      <div class="pp-h">
        <b>在线 {{ presence.users.length }} 人</b>
        <span v-if="editingCount">· 其中 {{ editingCount }} 人在编辑</span>
      </div>
      <div v-for="u in rows" :key="u.sid" class="pp-row" :class="{ fade: fading(u) }">
        <Avatar :uid="u.user" :name="u.displayName" :text="personNick(u.displayName)" :size="34" />
        <div class="pp-nm">
          <div class="pp-n1">
            {{ u.displayName }}
            <span v-if="u.self" class="pp-you">你</span>
            <span v-else-if="u.role" class="pp-role">{{ u.role }}</span>
          </div>
          <div class="pp-n2">{{ u.label || '—' }}</div>
        </div>
        <span class="pp-st" :class="u.mode === 'edit' ? 'e' : 'v'">{{ stateText(u) }}</span>
      </div>
    </div>
  </Popover>
</template>

<style scoped>
/* 宽度写死 = 满员宽度:4 × 30 + 3 × 3 间距 + 3 + 溢出 30 + 9 + 「8 人在线」≈ 60 + 内边距 18 → 256 */
.pb {
  width: 256px; height: 40px; box-sizing: border-box;
  display: flex; align-items: center; justify-content: flex-end; gap: 9px;
  padding: 0 12px 0 6px;
  border: none; border-radius: var(--radius-full);
  background: transparent; cursor: pointer;
  font-family: var(--font-sans);
  transition: background var(--dur-fast) var(--ease-standard);
}
.pb:hover { background: var(--bg-hover); }
.pb-stack { display: flex; align-items: center; gap: 3px; }

/* 环即状态,不另加图标:橙环=正在某一期的编辑态,白环=浏览中,淡=快掉线了 */
.pb-av.view { box-shadow: 0 0 0 2px var(--surface-white); }
.pb-av.edit { box-shadow: 0 0 0 2px var(--surface-white), 0 0 0 4px var(--hue-orange); }
.pb-av.fade { opacity: .5; }
.pb-more {
  display: inline-flex; align-items: center; justify-content: center;
  width: 30px; height: 30px; flex: 0 0 auto; border-radius: 50%;
  background: var(--surface-sunken); color: var(--text-secondary);
  font-family: var(--font-mono); font-size: 11px; font-weight: var(--fw-semibold);
  box-shadow: 0 0 0 2px var(--surface-white);
}
.pb-n { font-size: var(--fs-body); color: var(--text-secondary); white-space: nowrap; }
.pb-n b { font-family: var(--font-mono); font-weight: var(--fw-semibold); color: var(--text-primary); }
.pb-alone { color: var(--text-muted); }

/* L 以下(RESPONSIVE-LAYOUT-SPEC §3.2):头像组收成「人形图标 N 人」胶囊。同样定宽:
   人数只换字不换宽,顶栏不挪一个像素;点开名单的入口不变。 */
.pb-count { display: none; }
@media (max-width: 1280px) { /* L↓ */
  .pb {
    width: 76px; height: 34px; padding: 0; justify-content: center;
    border: 1px solid var(--border-subtle); background: var(--surface-white);
  }
  .pb-stack, .pb > .fp-tipw, .pb-n { display: none; }
  .pb-count {
    display: inline-flex; align-items: center; gap: 5px;
    font-size: var(--fs-body); color: var(--text-secondary);
  }
  .pb-count b { font-family: var(--font-mono); font-weight: var(--fw-semibold); color: var(--text-primary); }
}

.pp-h {
  display: flex; align-items: center; gap: 8px; padding: 12px 16px 10px;
  border-bottom: 1px solid var(--divider); font-size: var(--fs-label); color: var(--text-muted);
}
.pp-h b { color: var(--text-primary); font-weight: var(--fw-semibold); font-size: var(--fs-body); }
.pp-row { display: flex; align-items: center; gap: 12px; padding: 9px 16px; }
.pp-row + .pp-row { border-top: 1px solid rgba(28, 28, 28, .05); }
.pp-row:hover { background: var(--surface-card); }
.pp-row.fade { opacity: .6; }
.pp-nm { flex: 1; min-width: 0; }
.pp-n1 { display: flex; align-items: center; gap: 6px; font-size: var(--fs-body); font-weight: var(--fw-medium); white-space: nowrap; }
.pp-n2 {
  font-size: var(--fs-label); color: var(--text-muted); margin-top: 1px;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.pp-role {
  font-size: 11px; color: var(--text-muted); border: 1px solid var(--border-subtle);
  border-radius: var(--radius-full); padding: 0 6px; line-height: 16px; font-weight: var(--fw-regular);
}
.pp-you {
  font-size: 11px; background: var(--ink-900); color: #fff;
  border-radius: var(--radius-full); padding: 0 6px; line-height: 16px; font-weight: var(--fw-semibold);
}
.pp-st { flex: 0 0 auto; font-size: var(--fs-label); font-family: var(--font-mono); white-space: nowrap; }
.pp-st.e { color: var(--hue-orange); font-weight: var(--fw-semibold); }
.pp-st.v { color: var(--text-disabled); }
</style>
