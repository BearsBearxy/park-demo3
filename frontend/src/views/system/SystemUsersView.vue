    ...
    <!-- 数据体**不再整屏 v-if** —— 外壳常驻，只在叶子上放骨架（加载态设计稿 §07「精确占位」）。
      ...
      <div class="mx-body">
        <aside class="mx-kpirail">
          <KpiCard label="账号总数" :value="String(kpi.total)" sub="" tint="slate">
            <template #icon><component :is="iconFor('users')" :size="16" /></template>
          </KpiCard>
          <KpiCard label="启用中" :value="String(kpi.on)" sub="" tint="blue">
            <template #icon><component :is="iconFor('check-circle-2')" :size="16" /></template>
          </KpiCard>
          <KpiCard label="已停用" :value="String(kpi.off)" sub="登不进来,痕迹保留" tint="plain">
            <template #icon><component :is="iconFor('lock')" :size="16" /></template>
          </KpiCard>
          <KpiCard label="角色" :value="String(roles.length)" sub="" tint="cyan">
...
.su-in { width: 100%; box-sizing: border-box; height: 36px; padding: 0 12px; font-size: var(--fs-body); color: var(--text-primary); border: 1px solid var(--border-control); border-radius: var(--radius-md); outline: none; background: var(--surface-white); font-family: var(--font-sans); transition: border-color var(--dur-fast) var(--ease-standard); }
