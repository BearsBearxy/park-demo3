      ...
      <div class="av2-grid">
        ...
        <div class="av2-card av2-s12">
          ...
          <!-- I5(对抗复查):本分支唯一一条模拟带,原来是唯一一张没有自己口径浮层的带卡。
               ...
               F2(修复轮1,design-boards 对抗复查):下面这段浮层允许出现「80%」与「20%(18/90)」——
               这是这张卡自己判给自己的例外:两个数都是模拟分布的分位数/历史占比,不是回测校准声明
               (卡头两道文案门禁禁的是后者)。这条例外**不是**靠"门禁扫不到收起态的浮层"才成立——
               扫描已经改成会先点开浮层再看(expiryScreen.spec.ts F2),这两个数在断言里是被要求
               出现的,不是漏网之鱼。真正不许出现 % 的是本卡直接可见的读数句(.ana-read/.ana-ref)。 -->
          <AnaMethodNote>
