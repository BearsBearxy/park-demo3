...
export function rcGroupOption(periods: RcPeriod[]): object {
  ...
  return {
    ...
    series: [
      { name: '应收', type: 'bar', cursor: 'pointer', data: periods.map((p) => p.receivable / 1e4), barWidth: '26%', itemStyle: { color: mid, borderRadius: 3 } },
      { name: '实收', type: 'bar', cursor: 'pointer', data: periods.map((p) => p.collected / 1e4), barWidth: '26%', itemStyle: { color: blue, borderRadius: 3 } },
    ],
  }
}
