...
describe('fpNav', () => {
  it('has 4 layers and 49 items', () => {
    expect(FP_NAV).toHaveLength(4)
    expect(fpAllPages()).toHaveLength(49)
  })
  it('builds a route per item with layer back-refs', () => {
    const r = fpBuildRoutes()
    expect(Object.keys(r)).toHaveLength(49)
    ...
    // 系统管理层(RBAC-SPEC §4):不进 navLayers,可见性按 system:view;操作日志是 P2,本轮没有这一项
    expect(r['sys-users'].layer).toBe('system')
    expect(r['sys-roles'].layerLabel).toBe('系统管理')
    expect(r['sys-logs']).toBeUndefined()
  })
