...
### 3.4 最近动态（轻量降级）
跨 9 子系统取 `updated_at` 最新的 **6 条**记录 → 每条 `{ source(子系统名), period(acct_month/年月), time(updated_at, 'M/d HH:mm') }`。**无"谁"字段**（单管理员、无审计）。前端文案：「{source} · {period} 更新」。
...
4. 最近更新 — 全子系统 `max(updated_at)` 的 `HH:mm`，副 `{M/d} · {该记录子系统}`
