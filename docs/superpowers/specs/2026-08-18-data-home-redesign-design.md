...
- 抄表：**只给「已抄 N 块」，不给分母**。抄表屏那个 92/94 是 MeterView 前端 `cardCounts()` 在
  电水+分区筛选链上算的，后端另算一份分母必然与之漂移 —— 那正是 METRIC-SOURCE-SPEC §1 禁止的
  「同一判定两份实现」。比例留在抄表屏（它才有完整筛选口径），首页只回答「这步做没做、做了多少」；
