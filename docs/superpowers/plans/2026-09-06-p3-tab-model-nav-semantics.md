...
期望：2324 条全绿（本任务不加不减用例）。`anaAnomaly.spec` / `ledgerLeaveAndReturn.spec` 必须仍绿。
...
期望：2324 条全绿；tsc 零错；size-check 通过；`git status` 只剩本步待提交的文档。
...
| 项 | 结果 |
|---|---|
| vitest | **197 文件 / 2331 用例**全绿（起点 193 / 2266） |
| vue-tsc --noEmit | 0 错 |
