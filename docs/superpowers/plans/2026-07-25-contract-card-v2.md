...
Task 1-7 由 workflow 实现,全绿(前端 94 文件/786 测试,后端 429 测试)。另修 3 处:合同链空滚动条(`.cc-wrap` flex-shrink:0)、`today` UTC→本地、`rentTiers` 缺字段防崩。

**Task 8 阶梯回填完成**:
- 回源合同附图重提(81 屏,tiers[] schema)→ join by term_text + 公司名消歧 → **68 合同 / 163 档**入库 `contract_rent_tier`。
- dev 后端已重启(Flyway 55→56,DB_PORT=13306);detail 已下发 rentTiers;浏览器实测:周兴 S10-0074 两档正确、当前档(第二年起)高亮、§5.3 换档告警命中(合同现行 12.231 vs 当前档 14.282);力灏 S10-0192 相对期限正确降级。
- **9 户待核对**(未猜):旭化成/李富全/南宗/欧培仪/张文意/羊城科技/苏明东(demo3 无对应合同)+ 李李×2(歧义抢 S10-0149)。
- 回填 SQL/提取结果/join 报告在本会话 scratchpad;非 Flyway 迁移(contract_id 环境相关,一次性数据)。
