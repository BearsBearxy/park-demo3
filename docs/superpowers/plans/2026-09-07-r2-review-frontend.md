...
| **D-R2-6** | ~~store 按月一份，编辑闸也走它。~~ **执行中作废**，改成两条取数道（见 D-R2-10）。原文： 编辑闸要的是单键，但全月一趟（~20 键）比逐键 N 趟便宜，且屏切回来能命中缓存。四个动作之后失效当月。 |
...
| 编号 | 内容 |
|---|---|
| **D-R2-7** | **审核态拉不到 / 还没到 ⇒ 不挡编辑**，与旁边编辑锁的「拿不准就不进」**故意相反**。锁失灵会两人同改同保存、双方都提示成功（静默丢数据，不可逆）；审核态失灵后端 R1 那道闸照样拦，最坏是白录一次。而挡住的代价是把 12 个屏的编辑入口挂在一个额外端点上。已同步写进 CONCURRENCY-SPEC §3.5。 |
| **D-R2-8** | **闸只读已经到手的审核态，不在点击那一下 await 网络。** 头一版把取数放进 `toggle()`/`enter()` 的关键路径，jsdom 里直接红了 3 个屏 15 条既有用例（网络往返是宏任务，而那些用例只 flush 微任务），生产上的表现则是每次进编辑态多等一个往返。 |
| **D-R2-9** | **行动作作用于该行全部适用的键，不是单键。** 台账每公司一把、附10 每期区一把、附13/14 两个 scope、附7/8 两个 kind，一共占 19 把键里的 8 把 —— spec §7.5 只给 chips 配了颜色，没说这 8 把的动作挂哪儿。 |
| **D-R2-10** | **闸道 / 清单道两条取数道**（替代 D-R2-6）。`GET /api/review/states?year=` 只发已落库的行、不跑首页聚合，12 个编辑入口都走它；`GET /api/review?period=` 保留给本月出账屏。起因是年表屏按月份行上锁（D18）要整年 12 个月的审核态，走清单道等于跑 12 遍首页聚合。 |
| **D-R2-11** | **编辑入口是三条不是两条**（D-R2-2 写的时候只找到两条）：第三条是 `LedgerWideTable`，宿主 `LedgerView` 是裸的 `const edit = ref(false)`。 |
| **D-R2-12** | **顶栏上下文 chip 的审核态图标不做**（§7.5 原文有）。药丸就在同一条工具栏上、说的是同一件事；要做它就得在 `Toolbar.vue` 里再抄一份「屏 → kind」表，正是 `reviewGateCoverage.spec` 存在的理由。 |
| **D-R2-13** | **体积预算 3900 → 3950，签字上调。** master 实测 3896.3（只剩 3.7KB），R2 净 +22.0KB 且首屏 index 只涨 0.2KB。理由与实测数写进 `size-check.mjs`。 |
