...
| E-9 | **R-4 的一个副作用，等用户裁定** | `AllocService.createRule` 带初始分母时会 `saveCfg(scope="rule:<新id>", key="coefficient", acctMonth="")` → `ParamService.write` → `assertNoLockedMonth(PARAMS, null)`。于是 **params 只要有任一月 submitted/approved，「新建公摊池（带初始分母）」就永久 423**，文案还说的是「计费参数已审核」。试过按「这个键没有历史行就放行」收窄——**不成立**：key 是 `coefficient`，它在别的池下有大量行；改按 (key, scope) 判又会在 `scope="p1"` 这类情况下放行真正的口径修改。**2026-09-07 用户拍板 B：新建池那两处显式豁免**。落法：`ParamService.write` 加第三参 `newPoolBootstrap`（唯一传 true 的调用方是 `AllocService.createRule` 的两次 `saveCfg`），只跳过长期默认行那道闸。口子只对**建**开不对**改**开——池建成后再动这两个键走参数页的普通 write，照样被 R-4 拦；`ReviewGuardChainIT` 一正一反两条钉住这个边界 |
