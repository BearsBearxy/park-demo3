...
### §1.1b `W_METER_BIND_STALE` 表绑的合同本月用不上
...
| 对账单影响 | 行照出、金额一分不变；`contract_id` 快照落的是**本月并未生效的那一份**；钉的是别户的合同时（METER-TIMELINE-SPEC §3.6 不采用）快照为空 |
...
### §1.1c `W_METER_BILLED_ELSEWHERE` 表已在别户的单上收过

> 2026-09-24 补的第十类(METER-TIMELINE-SPEC §5「催缴单生成」)。编号用 `1.1c` 的理由同 §1.1b。
> `WARN_CODES` 里排**第三**,紧跟两类表的问题。
...
| 条目形状 | `{hint}`(hint = `meterTag(m)`,与 §1.1 同一条判据);payload = `meterId` |
| 落点屏 | `/meters` 园区抄表 · 动作「去园区抄表」 |
| 判据(代码) | `lockedMeters.contains(m.getId()) && !lockedTenants.contains(tid)` —— `BillNoticeService.generate` 表循环第一句;`lockedMeters` = 本月 confirmed/exported/issued 单明细里的 `meter_id`(与 `lockedTenants` 同一组状态) |
| 判据(人话) | 这块表本月已经在一张锁定的单上收过,而档案现在把它挂在另一户名下 |
| 对账单影响 | 这一户的草稿**不出这块表的任何行**(电 / 水 / 管理费),量只在已锁的那张单上算一次;金额 = 少掉这块表的那几行 |
| 实测 | 2026-09-24 dev 库(V128 未跑,按 meter 现值数,与迁移后 assignAt 同值):锁定单 7 张(2024-02 exported)明细含 11 块表,现挂别户的 **0 块** → 0 条(F 阶段收口时独立复数,同数)。库里 0 条不是不加的理由:它防的是表档案按月记录之后「换户后重生成」那一下(J5-1 双收) |
| 进抽屉 | 是 |

> **为什么界面上还会走到这一步。** 改表档案的写路径(手改 / 状态 / 合同终止 / 撤销导入)都查冻结:
> 含这块表的单已确认 / 已导出的月份一律拒改。这一条是生成侧的兜底 —— 上线前就改过的历史数据、
> 绕过闸的脚本写,都靠它挡住双收。钉它的 IT:`BillNoticeApiIT.t40`(payload / hint 分两列存,
> 作废那张单再重生成后这块表回到新户、告警消失)。
