...
| **L1 定位** | 白底 1px 边 + 卡头，**一根坐标轴都没有** | 强调色 `#9D5D17` 只在 L0/L1 |
| **L2 佐证** | 白底 + 卡头 + **坐标轴** | 坐标轴只在 L2 与抽屉出现 |
| **L3 下钻** | 720px 覆盖层 + 遮罩，内部**不再包卡** | 全屏唯一的表格与统计图形 |

**四条可验收的排他规则**：26px 只在 B0 / `#9D5D17` 只在 L0-L1 / 坐标轴只在 L2 与抽屉 /
`height=440` 整屏一次都不用。
...
点队列任意一行 / B4 任意一条线段 / B5 任意一条 → 打开**同一个**抽屉，URL 加 `#st={id}` 不换路由。
...
10. 26px 只在 B0 出现一次；`#9D5D17` 不出现在 L2/L3；坐标轴不出现在 L0/L1；
    `height=440` 整屏零次。
