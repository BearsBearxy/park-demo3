...
| L2 | 残差自相关 ACF | ECharts 柱，滞后 0–30 | 200 | **「为什么不用 √N 而用块自助」的证据** |
| L3 | 残差 vs 年积日 | ECharts 散点 | 250 | 季节成分抛干净了吗 |
| L4 | 块自助零分布 | ECharts 直方图 + 观测竖线 | 200 | 观测值在零假设下多离谱 |
| L5 | 抛光收敛诊断 | DOM 文字读数 | — | 迭代收敛了没 |
| L6 | 数据质量矩阵 | **CSS Grid 手写**，13 行 × 整段日历 | — | 到底剔了哪些天 |
...
| **L6** 质量矩阵 | **跟着显示段**（2026-09-02 起） | **它是记账不是估计** —— 一格一天，没有样本量门槛。<br>365 列 @2px 认不出哪格是哪天，31 列才谈得上「回答剔了哪些天」 |
| **L3** 残差 vs 年积日 | 整年，**当段的点加重** | 它的横轴**就是**年积日：一个月是这条弧上的一小截，<br>单看那截读不出季节成分。整年画完再把当段点画深，两件事都成立 |
| **L2** ACF | 整年，滞后上限 `min(30, ⌊n/4⌋)` | 块自助的块长是 14 天。**要判断块长够不够，就得看得见滞后 ≥14**，<br>一个月只诚实支持到滞后 7 —— 那时这张图已经答不了它存在的那个问题 |
| **L1** α · **L5** 收敛 · **L7** 的 α / N_eff / 变点 | 整年 | 中位抛光按构造拟在整年上；月档 α 大体重复 B1 队列和大图已经说过的话 |
...
| 文件 | 内容 |
|---|---|
| `src/views/analysis/PvQualityGrid.vue` | §06.5b L6 数据质量矩阵。CSS Grid 手写，四态 |
