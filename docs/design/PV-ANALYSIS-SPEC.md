...
- **色**：屏上 3 个色相，首屏只有 2 个。墨阶 `--ink-900/700/500/300/100/050`；
  出范围 `#9D5D17`（`--hue-orange`，对白底 5.23:1）**上越下越共用这一个色，方向靠位置 + ▲▼**；
  链接 `#3675B2`（`--hue-blue`）**永不进数据**；账面量段 `#788CB0 / #A0CDE8 / #C4E2F4` 同一蓝家族三档明度。
  **13 栋没有任何一栋拥有自己的颜色。**
  不用红：这屏禁严重度判词，红黄绿三色灯本身就是判词；且同屏有收益金额，大陆红=涨与监控红=险必撞车。
