...
**期间两档：按月逐日 / 按年逐月。** 模型永远吃全年，只有「画哪一段」跟着期间变 ——
模型要历史，屏要新鲜。**月段只画当月那三十来个点**，逐日铺满全年没人看得过来。

| # | 块 | 图种与编码 | 实现注意（不上屏） |
|---|---|---|---|
| B1 | **逐段看板**（这屏的重心） | 每栋一行：线 = 这栋当刻度发电 ÷ 全园同刻度中位；淡带 = 这栋自己的正常范围；出范围的点单独标色。所有行**共用同一段纵轴**，缺抄的刻度**断开不连线** | **这条线里没有任何模型**：分子分母都是实测度数，反事实的「应发/缺口」一个字不出现。各栋规模不同所以比值本身有高有低 —— 带是它**自己**的历史范围，规模差异自动消掉，连装机容量都不用。用内联 SVG，不用 ECharts。**日抄与月抄分两组**：N=12 与 N=244 的可信度差一个量级 |
| F1 | **这一段发生了什么** | 列表，每行一句事实：日期、刻度数、方向 | 断崖（连续同侧）与波动（散在两侧）**措辞上必须分开** —— 那正是用户要的两种感觉。形态判在前、连续段判在后：只看「有没有连续 N 个同侧」会把剧烈上下跳的栋说成断崖（实测：31 天里 22 天出带、上下各半，只因中间连着 4 天同侧就被报成断崖）。散着出带的门槛要从带宽推，不能直接用 `pv_band_run` |
