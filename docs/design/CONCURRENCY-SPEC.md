...
**两者「拿不准」时的口径故意相反**（R2 实施时拍板，2026-09-07）：
- 锁拿不到 / 拿不准 → **不进**（`useEditLock` 一路如此）。锁失灵的后果是两人同改同保存、后保存的整片覆盖前一个，而且**双方都提示保存成功** —— 静默丢数据，不可逆。
- 审核态拿不到 → **放行**（`stores/review.ts` 的 `blockOf`）。后端 `ReviewGuard` 那道闸照样拦，用户拿到的是一句准话；最坏结果是白录一次，不是丢数据。而挡住的代价是把 12 个屏的编辑入口挂在一个额外端点上。

改任一条之前先读这一段：两个「拿不准」看着像同一件事，处置反了会各自出各自的问题。
