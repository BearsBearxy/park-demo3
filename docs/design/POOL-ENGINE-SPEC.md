...
**alloc_loss_note（V127，人工备注）**：`(ym, head_building_id, note)` + `uk_loss_note(ym, head_building_id)`。**不是 alloc_loss_result 的一列** —— 那张快照 generate 时按 ym 先删后插重建（`computeLossUnits` 纯派生不读旧行），抄表批量删也整月抹掉它，备注落上去每重算一次就归零。独立表挂业务键，generate 一格不碰，重算天然存活（手法同 V92 `bill_note_override`）。清空备注=删行，表里不存空串行。
写口校两道：楼栋存在，**且它在该月真是 `alloc_loss_result` 的组头**（否则 404）—— 只校前者的话，写到非组头的楼栋上会 200 落库却永远贴不回屏上，调用方拿不到任何反馈。

> **已知的缝（未修，对抗复查 2026-09-23）**：键里的 `head_building_id` 不是常量。`loss_head` 是版本链（按 `acct_month<=ym` 取最大者前滚），楼栋分组**会随月变**—— 库里 `building:30 loss_head` 默认 34、另有一行 `acct_month=2023-10` 值 30，对应 2023-08 的 head 集合无 30、2024-02 的 30 与 34 并存。在参数中心改动 `loss_head` 的版本行后重算，那栋并回另一个组头，屏上那条备注会当场消失且无任何提示，`alloc_loss_note` 里的旧行永久残留（版本行改回去旧备注又原样冒出来）。没做清理/对账/告警：改 `loss_head` 是罕见的管理动作，专门为它建一套搜孤机制不值。真要管，最小的做法是重算后把孤行数报到屏上的告警抽屉里。
...
| GET /api/alloc/loss?ym= | 损耗表：单元行(快照，含人工备注) + 对账区(读时派生:供电侧总表/合计/两级损耗，每期区 1~2 行) + 铝缆明细 |
| PUT /api/alloc/loss/note | 写楼栋损耗备注（按 ym+组头楼栋 upsert；空串=删行）。落独立表 `alloc_loss_note`（V127），generate 不碰它 —— 重算/重导读数后备注仍在。权限走 `/api/alloc/**` 的 BILLING_RUN_EDIT（与 generate 同档）；守卫 `ReviewKind.ALLOC_LOSS`（月审过/锁账后不许再改字） |
...
- 列：位置 | 总表用电量 | 铝缆用电量(p2) | 分表用电量 | 损耗量 | 原损耗率 | 公摊分摊度数(p1) | 调整度数 | 调整损耗 | 收取租户损耗率 | 备注；合计行 + 对账区(见 §6.2)
- 编辑态：**只有「备注」一格可写**（PUT /alloc/loss/note）。其余列全是派生值，口径（核算方式/归组/总表取数/不计入的表/调整度数/损耗率加点/手工指定率）仍在计费参数页 ③ 核算口径改（S21 §5.6），改完回来重新生成

**对账区：一个期区 1~2 行**（用户 2026-09-23 报障："合计行 101604.40，对账行 63054.40，屏上没一个字说 A座 不在里面"）

- 对账行来自后端 `AllocPoolDTOs.LossRecon`，每条渲染成两行（vs 总表合计 / vs 分表合计），标签读作 `{supplyLabel} vs {sumLabel}总表合计`：
  - **第一条**＝供电局那块期区总表（`zone.loss_supply_meter`：一期 meter 266「B-G座总电」、二期 meter 1「二期总电」）对上它管得着的那几栋。`building:{head}.loss_recon=0` 的单元不进合计（库里只有 `building:13` 一期A座）。`supplyLabel`＝那块表在库里的名字；`sumLabel`＝有排除项时 `除{被排除栋}外各栋`，否则 `各栋`。
  - **第二条**（仅当该期区确有 `loss_recon=0` 的单元时才发，二期没有就不多这一行）＝把被排除的栋并回来的全部楼栋：供电侧＝期区总表读数 ＋ Σ 被排除单元的 `c_qty`；合计＝该期区**全部**单元的 `c_qty` / `d_qty`（与屏底合计行同口径）。`supplyLabel`＝`B-G座总电 + A座总电`，`sumLabel`＝`全部楼栋`。
  - **能把被排除单元的 `c_qty` 当供电侧加进来的前提**：那栋的 C 就钉死取它自己那块供电局表（`building:{head}.loss_c_meter`，一期A座=meter 179「A座总电」）。没钉 `loss_c_meter` 的栋，C 是自家总表Σ、不是供电局读数，第二条就不发。
  - 损耗量＝合计 − 供电侧，率＝损耗量 ÷ 供电侧，两条同式。
  - **两道守卫**（对抗复查 2026-09-23）：
    - 该期区的单元被排光了（进合计的单元数＝0）→ **第一条不发**。不挡的话屏上会出「… vs 除X外各栋总表合计 = 0.00，损耗量 = −整个供电量，原损耗率 −100%」。库里 2023-10 的一期就是这个形状（只有 A座 一行、而 A座 正是被排的那栋），只是恰好被「期区供电表当月无读数」挡住了。
    - 第二条的供电侧（期区表 ＋ Σ 被排单元 c_qty）**为 0 就不发**。`MeterService.usage` 不夹负（换表/回零会出负用量），刚好抵消成 0 时做除数会 `ArithmeticException: Division undefined` → GET /alloc/loss 整条 500、整屏打不开。第一条本来就有这道守卫（`supply.signum()==0` 就 continue），第二条漏了。
  - 遗留的口（**未修**）：放行第二条的判据只是「这栋配了 loss_c_meter 且那块表还在」。库里区分不了供电局表与楼内自装总表（meter 179 / 266 的 ownership 都是 `infra`）。把 `loss_c_meter` 指到一块自装总表，这一行会把一块下游子表当独立供电源加进供电侧。缓和的是 `supplyLabel` 就写着那几块表的名字，人能自己核。要真钉死得在参数中心写 `loss_c_meter` 时校那块表的来源。
- 2024-02 一期锚点：第一条 68320.00 / 63054.40 / 62923.96；第二条 106870.00（=68320.00+38550.00）/ 101604.40（损耗量 −5265.60，率 −0.0493）/ 97800.46（损耗量 −9069.54，率 −0.0849）。
...
## 9. 告警可操作性铁律（2026-08-14 用户拍板）

> 用户原话：「每次用户导入的时候不希望出现无法手动修改的问题，这样的话用户怎么用」。
> 2023-08 那次生成 61 条告警，暴露出闭环有三处断点 —— 系统报了问题，但用户在界面上无法自己修掉。

**铁律：generate 报出的每一条告警，都必须能在界面上被用户自己修掉。**
落到三条可检查的要求：

1. **点名，不报编号也不只报数。**
   `受益租户#371` 这类内部 id 用户在任何屏上都搜不到，等于没有修复路径；
   `有 125 户缺起止日期` 这种只报数的，用户要在 431 份合同里肉眼捞。
   → 逐户告警一律走 `ctx.nameByTenant()`；聚合告警走 `AllocService.suffix()`（前 8 个名字 + 「等」）。
2. **指路：告警文案里点名说去哪个屏、哪个筛选、哪一段。**
   括号里的筛选名必须与目标屏的按钮文案**逐字相同**（改一边必须改另一边）。
   现有三处落点：合同页「缺起止日期」/「无租金计费行」/「租金行未绑单元」三个缺口 chip。
3. **给目标值，不只给差额。**
   「基数与名册Σ不一致」若只报不一致，用户在抽屉里仍不知道该填什么；
   现在直接算出在租名册面积Σ 并写进文案（"请把基数改成 74760.35"）。

4. **报出来的东西必须真的需要用户动手**（2026-09-23 补，用户报障）。
   铁律 1-3 管的是「报得出、找得到、改得了」，这一条管**该不该报**：
   一条提示如果照着做完，重算之后什么都不变，那它就是噪音，而且是每个月都来一次的噪音。
   实测两类：

   | 提示 | 照做之后 | 处置 |
   |---|---|---|
   | 电梯池报首层租户「新在租」 | 引擎 `skip1F` 把首层桶整桶剔掉，勾进去该户金额仍是 0，下月原样再报 | `memberDiff` 的 `added` 对 `share_elec_elevator` 池过滤掉**在本栋单元全在首层**的户。跨层户（1F+3F）照样用电梯，照报 |
   | 「已退租」报的是下月才起租的户 / 一份合同都没有的户 | 照做会把还没进场的人从池里摘掉 | `inForceState` 从三态拆成五态，`removed` 只收真退租 |

   **在租五态**（`AllocService.inForceState`，取最优 `yes > unknown > future > none > no`）：

   | 态 | 判据 | 屏上 | 进不进「已退租」 |
   |---|---|---|---|
   | `yes` | 有非草稿合同起止齐全且与该月重叠 | 在租 | 否 |
   | `unknown` | 有非草稿合同，但能判的那几份缺起止日期 | 合同缺起止日期 | 否（2026-07-30 已摘出） |
   | `future` | 有日期的合同全都在该月之后才起租 | 还没进场 | **否**（2026-09-23 摘出） |
   | `none` | 一份非草稿合同都没有（含 `Roster.stateOf` 查不到的兜底） | 没有合同档案 | **否**（2026-09-23 摘出） |
   | `no` | 有日期的合同全都在该月之前到期 | 已退租 | 是 |

   **实测（改前）**：2023-08 / 2023-10 / 2024-02 三个月共 84 条「已退租」提示，**真退租 0 条** ——
   81 条是「下月才起租」（尧萍 S10-0112 起租 2024-03-01，却在 2024-02 被报已退租），3 条是零合同档案
   （吴跃平#335，每月一次）。同期库里确有 124 户所有合同都已到期，但没有一户在任何池的名单里。
   电梯池首层那一类 2024-02 有 11 条（rule 3/8/22/50），而这几个池现有受益人的楼层实测全在 2~7 楼。

   断言：`AllocServiceTest.inForceState_fiveWay`（五态逐个 + 多合同取最优）、
   `AllocApiIT.memberDiff_addedRemoved`（三态各不相同 / `removed` 只收 `no` / 电梯池压首层但楼层公共池不压 /
   电梯池的非首层户照报）。五条各自破坏验证过。

**配套的两个界面缺口已补**（同日）：

| 缺口 | 补法 |
|---|---|
| 合同页无法筛出「该补档案」的那批 | `ContractDTO` 加 `billingLineCount` / `unboundTermCount`（**都只数建筑类租金行**，与引擎读的那一档同口径），合同页出缺口 chip 条，补完自动消失 |
| `billing_term_unit` 行级绑定**全站只读**（3 个 Service 读、无一处写、前端零界面），告警却写着「请补绑定」 | `BillingLineReq/DTO` 加 `unitIds`；合同编辑弹窗每个标的段出「面积落在」单元 chips。`unitIds=null` 保持旧的按键回挂快照语义，导入路径与老客户端不受影响 |

**未做（留给用户拍板）**：12 条「Σ层份 与账册系数 T 不符」里有 8 条是负差，
代码注释已判定为「账册刻意欠配＝空置园区自担」。它们**能**改（抽屉份额列/系数簿），
只是不该需要改 —— 要不要从告警降级为说明，是口径决策不是修 bug，没动。
