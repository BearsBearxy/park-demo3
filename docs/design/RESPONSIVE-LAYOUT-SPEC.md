# 响应式布局规范（RESPONSIVE-LAYOUT-SPEC）v1.2
...
全站 49 屏收敛为九类（2026-09-21 从六类扩到九类，见下）。**新功能屏先认领模式，再按行取规则**；
...
> 新增的 §5.7 / §5.8 / §5.9 先把「这是一类」和「验收判据」定下来，具体形态等画板回填。
...
### 5.7 屏顶 KPI / 统计卡行（2026-09-21 新增一类，规则待稿）
...
**判据先写死一条，具体形态等稿**：这一类在 S 档的验收标准不是「卡片好不好看」，
而是**收完之后主内容（表 / 图）能不能进首屏**。规则见「手机模式层」画板，画完回填。

### 5.8 年 × 12 月选期矩阵（2026-09-21 新增一类，规则待稿）
...
**写 `minmax(0, 1fr)` 只能让它不撑破容器，月卡会窄到读不出字**——真解法是降列或换形态，
规则见「手机模式层」画板，画完回填。

### 5.9 屏顶工序条 `FPStepStrip`（2026-09-21 新增一类，规则待稿）
...
它承载的信息是「我在整条流程的第几步」，收的时候这个信息不能丢。规则见画板，画完回填。
...
**第 7 条和 `TemplateEditorPanel.vue:595` 的 232px 版本链不能照抄 chips**：
前者在弹窗内部（可用宽已经被弹窗壳定死），后者是**纵向历史**不是平级选择，
收成一行 chips 会把「先后顺序」这个信息丢掉。这两种的收法见
「手机模式层」画板（2026-09-21 出稿中），画完回填到本节。
