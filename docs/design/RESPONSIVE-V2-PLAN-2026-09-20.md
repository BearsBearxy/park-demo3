...
### P5 · 分析层手机落地页「查一个数」 —— **不做（2026-09-21 用户拍板）**

> 拍板原话：**「保持落驾驶舱」**。手机上点底栏「分析」仍然落 `cockpit`，与桌面一致。
>
> 实现做完过一版（三枚数瓦 + 搜索框 + 「全部分析」），留在已推的 `jfen/responsive-v2`
> 分支 `c2f50f3`，要捡随时能捡。那一版查证出来的三条事实值得留着，下次再提这块先看：
>
> - **卡级锚点全仓不存在**，只有 `PvMeterAnaView` 有它自己的 `#st=`；
> - `cockpit.logic.ts:357` 原话「落分析屏的三条 p 今天不被消费（usePeriod 单例，spec §12 遗留）」
>   —— 所以稿的「点一枚瓦 = router.push + # 锚到那张卡」两半都做不到；
> - `tabs.ts:84` 的 `recent` 是 `ref<string[]>` 只存屏名，带上下文的 `ctx` 注释写着
>   「只在内存」且 `close()` 当场清空 —— 稿要的「最近看过」拼不出来。
>
> 另有一条实现教训：新建的手机屏根节点**必须挂 `.fp-fluid`**，否则 `base.css:130`
> 在 ≤960 兜的 `min-width:800px`（§8 屏级地板）会把整页撑到 800 宽，390 上右边一半出屏。
> 浏览器实测：不挂时根 800、第二枚瓦 left 404→right 800；挂上后根 375、那枚瓦 191→375。

<details><summary>原计划（已作废，留档）</summary>
...
| 20 | ds/Select 底部面板的标题行读字段名（稿 FormPhone 画的是「期数」） | 没传 `label` 时退成「请选择」。全仓 **69** 个 `<Select>` 调用点，只有约 20 处传了 `label`；稿只画了一处。补全是一次全仓扫，不是 P4 的活，而「请选择」是句真话不是错话。真要做，连同 `aria-labelledby` 一起定，单开一条 |
| 21 | 触屏固定钮挂到**每一个**页签上 | 稿（ShellTablet）只写「Pin 钮常显即唯一入口」，图上一颗固定钮都没画，尺寸与落位没给。给每签都挂的代价算得出来：`TAB.MIN=96`，扣图标 16 + 固定钮 20 + 关闭钮 20 + padding 16 + 三处 gap，标题只剩个位数像素——页签上认不出是哪一屏。改成**只挂当前签与已固定签**：「唯一入口」要的是「想固定当前这屏时有地方点」，已固定签留着是为了取消固定也有入口（固定签不渲标题，不抢宽） |
